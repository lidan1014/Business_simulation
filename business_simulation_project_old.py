...
class Business_simulation:
    ...
    def each_customer_buy_0_1(self, buying_id, shopping_df, product_information_df):
        buying_or_not = []
        for i in range(int(shopping_df.ix[buying_id][2])):
            gender = shopping_df.ix[buying_id][1]
            x = shopping_df.ix[buying_id][3 + int(i)]
            category = product_information_df.ix[int(x)]['category']
            price = product_information_df.ix[int(x)]['prices']
    ...
    def simulate_whole_data(self, simulated_buying_0_1, shopping_df):
        ...
        for item in list(range(0, self.num_customer)):
            ...
            # get the product id
            for i in range(simulated_buying_0_1['session_length'][id]):
                customer_list.append(int(shopping_df.ix[id][0]))
            # get the orginal index
            for i in range(simulated_buying_0_1['session_length'][id]):
                session_length_list.append(id)
            buy01 = simulated_buying_0_1.ix[id][3:int(3 + simulated_buying_0_1['session_length'][id])].values
            product_id_i = shopping_df.ix[id][3:int(3 + simulated_buying_0_1['session_length'][id])].values
